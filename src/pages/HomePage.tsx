...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const userProfile = await HomeService.getUserProfile();
  ...
  return (
    <Layout username={username} email={email}>
      <div className="max-w-5xl mx-auto">
        <header className="mb-8">
          <h1 className="text-4xl font-bold">반갑습니다</h1>
          <p className="text-gray-600 mt-2">{username} 님의 나누아이디 사용을 환영합니다</p>
        </header>

        <h1 className="text-xl font-primary mb-2">추천 설정</h1>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {statusList.map((status, index) => (
            <StatusCard key={index} title={status.title} subtitle={status.subtitle} active={status.active} />
          ))}
        </div>
